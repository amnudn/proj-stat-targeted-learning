...
% Bullet to end example/definition environments
\newcommand\xqed[1]{%
...
\begin{example}[ATE]
  ...
  \fxnote*{jeg skal bruge \(\mathcal{P}\) for a aligne med Anders'
    notation?}{\(\Psi \, : \, \mathcal{P} \rightarrow \R\)} of
  distributions \(P\in\mathcal{M}\), for our purposes defined as
\begin{align}
  \Psi( \mu_X, f) = \int \big( f( 1, x) - f(0, x) \big) d\mu_X (x), 
  %\EE_P \big[ \EE_P[ Y \mid A=1, X] - \EE[ Y\mid A=0,X]\big],
  \label{eq:Psi:ate}
\end{align}
where \(f (a, x) = \EE [ Y \mid A=a, X=x]\) and \(\mu_X \) is the
marginal distribution of \(X\).
% where \(\EE_P[\cdot]\) denotes the
% expectation operator under the distribution \(P\in
% \mathcal{M}\).
\xend
\end{example}

% \fxnote*{introduce briefly}{the ATE ...}
...
into the integral operator
  ...
  optimal bandwidth estimator. \fxnote*{Add explicit bias variance
    decomposition?}{This example is in fact simply enough to allow an
    exact analytic calculation of the bias and variance of the target
    parameter, and hence it is fairly straightforward to
    mathematically prove the behavior suggested by
    figure~\ref{fig:kernel-undersmoothed}.}  %\xend
...
%\end{example}
  ...
  happens if we use \fxnote*{todo}{the first estimator suggested
    above}.
...
map $\nu \mapsto \TT(\P, \nu)$, where $\P$ is held fixed, and \fxnote*{can we make this more
  practical and give an example?}{ $\Vert \blank \Vert$} is a norm defined on the space in which the
nuisance estimator $\hat{\nu}_n$ takes it values -- typically a function space. Using empirical
process theory or sample spitting, it can in many cases be shown that \fxnote*{\(\mathbb{G}_n\)
  needs to be defined}{}$\G{\phi( \blank, \hat{\nu}_n)} = \G{\phi( \blank, \nu)} + \smallO_{\P}(1)$
\citep{van1996weak,van2000asymptotic,chernozhukov2018double}. Furthermore, if we assume that
$\hat{\nu}_n$ can be estimated at rate $n^{-1/4}$, meaning that \fxnote*{\(\smallO_{\P}\) needs to
...
the example converges \fxnote*{check}{at rate $n^{-2/5}$} is what
...
estimator, so this strategy is not applicable in practice \fxnote*{ref
  to undersmoothed HAL}{(though see ...)}. If we want to use flexible
...
derivative, and then indicate how this can be used \fxnote*{jeg er
  dum, det forstår jeg ikke? -> parametrization}{to design $\TT$} with
partial derivate equal to 0. In section~\ref{sec:estim-inform-bounds}
...
\fxnote*{hely would like to change this
  notation}{$\partial_0f(\epsilon)$} for maps $f$ with a real domain
...
is used by \cite{chernozhukov2018double} to define \textit{Neyman
  orthogonality}: A function \fxnote*{define the sample space
  $\mathcal{Z}$}{$\phi \colon \mathcal{Z} \times
  \mathcal{V}\rightarrow \R$} fulfills the Neyman orthogonality
condition \fxnote*{proper def?}{(wrt.\ $\mathcal{V}$)} if the map (as
...
instance, a surface embedded in $\R^3$, \fxnote*{Make the picture}{see
  picture}).
...
\begin{definition}[Canonical gradient]
  ...
  $\dot{\TT}_{\P}$ as the \fxnote*{first time canonical gradient is
    mentioned, I think we need motivation, c.f., above
...
\begin{proposition}
  ...
  Let $(\mathcal{P}, \TT)$ be a statistical problem \fxnote*{canonical
    gradient not used in the statement this proposition? perhaps add
    extra equality to (6)?}{with canonical gradient}
...
\begin{proof}
  The \fxnote*{introduce or mention before}{chain rule for Hadamard
...
also \fxnote*{}{identify} the function $\phi_{\P}$ with the canonical
...
% \subsection{Debiasing and efficiency}
...
\fxnote*{wait, is the decomposition needed for this result, or can we
  wait with returning to the decomposition till talking about
  estimation?}{Returning to the decomposition in
  \eqref{eq:target-decom} (Section \ref{sec:motivation}) we have the
  following results.}
...
\begin{remark}
  ...
  $\phi(\blank, \nu(\P)) \in \Gamma_{\P}$; hence, \fxnote*{I think
    this must be correct}{any gradient for the statistical problem
    will fulfill the Neyman orthogonality condition} (under regularity
  conditions). \xend
\end{remark}

\fxnote*{I sort of feel like moving this to the next
  section?}{\textit{Debiasing property of an estimator based on the
    canonical gradient.}}  \fxnote*{It is not until here we need the
  decomposition of \eqref{eq:target-decom}, am I right?}{The result
  shows that estimators} based on the canonical gradient has a
...
are not estimable at this rate themselves. \fxnote*{What is the
  intuitive explanation of this result? In standard calculus, the
  derivatives
  are orthogonal -- can this intuition explain the results?}{} \\
...
\fxnote*{proposed change to section title here}{If you agree on the
  above (moving the debiasing remark down)}, I think we should return
