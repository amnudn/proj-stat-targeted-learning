...
$\P$ is held fixed, and \fxnote*{can we make this more practical and
  give an example?}{ $\Vert \blank \Vert$} is a norm defined on the
...
spitting, it can in many cases be shown that \fxnote*{\(\mathbb{G}_n\)
  needs to be
  defined}{}$\G{\phi( \blank, \hat{\nu}_n)} = \G{\phi( \blank, \nu)} +
\smallO_{\P}(1)$
...
meaning that \fxnote*{\(\smallO_{\P}\) needs to be
